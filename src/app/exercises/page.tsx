...
export default function StudentsPage() {
  ...
  // ✅ NUEVO - Estado para validación de email
  ...
  // ✅ NUEVO - Función para validar dominio de email
  const validateEmailDomain = (email: string): boolean => {
    const emailRegex = /^[a-zA-Z0-9._-]+@uyCoding\.com$/;
    return emailRegex.test(email);
  };

  // ✅ NUEVO - Handler para cambio de email
  const handleEmailChange = (email: string) => {
    ...
    if (email.trim()) {
      if (!validateEmailDomain(email)) {
        setEmailError('El correo debe terminar en @uyCoding.com');
      } else {
        setEmailError('');
      }
    } else {
  ...
  const handleCreateStudent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.message && error.message.includes('@uyCoding.com')) {
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleCreateStudent} className="p-8 space-y-5">
              <div className="grid md:grid-cols-2 gap-5">
                ...
                {/* ✅ MODIFICADO - Input de email con validación */}
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      placeholder="nombre@uyCoding.com"
                      className={`w-full pl-11 pr-4 py-3.5 bg-slate-50 border ${emailError ? 'border-red-300 focus:ring-red-500' : 'border-slate-200 focus:ring-indigo-500'} rounded-xl focus:ring-2 focus:border-transparent outline-none transition-all text-sm font-medium text-slate-700 placeholder:text-slate-400`}
                    />
                  </div>
                  {emailError && (
                    <p className="text-xs text-red-600 font-medium ml-1 flex items-center gap-1">
